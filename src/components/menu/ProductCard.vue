<template>
  <div
    ...
    <div class="relative overflow-hidden bg-gray-100 rounded-t-xl">
      ...
      <!-- Stock Status -->
      <div v-if="product.inventory.status === 'out_of_stock'"
    ...
    <div class="p-4">
      ...
      <div class="flex items-center space-x-1 my-3">
        <div class="w-2 h-2 rounded-full" :class="getStockStatusClass(product.inventory.status)"></div>
      ...
      <!-- Action Button -->
      <button @click.stop="handleProductClick"
        class="w-full mt-4 py-2 px-4 bg-violet-500 text-white rounded-lg hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium cursor-pointer"
